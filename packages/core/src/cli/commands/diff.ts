...
export async function diffCommand(configPath: string): Promise<void> {
  try {
    ...
    for (const entry of result.manifest.files) {
      ...
      const genFile = result.outputs.get(entry.generator)?.files.find((f) => f.path === entry.path)

      if (!genFile) continue

      const generatedContent =
        (genFile.header ?? `// @generated by newel — do not edit\n`) + genFile.content
      ...
      console.log(unifiedDiff(entry.path, current.split('\n'), generatedContent.split('\n')))
      console.log()
    }
