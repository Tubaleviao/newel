...
import { toSnakeCase, tableName, enumTypeName, slugify } from '@newel/core'
...
export class SqlGenerator implements Generator {
  ...
  async generate(schema: FabricSchema, ctx: GeneratorContext): Promise<GeneratorOutput> {
    ...
    files.push({
      ...
      header: '-- @generated by @newel/generator-sql — do not edit\n',
    })
    ...
    if (!prevSnapshot) {
      ...
      // Scan the migrations directory on disk — ctx.outputs is not yet populated
      // when this generator runs, so it cannot be used to enumerate prior files.
